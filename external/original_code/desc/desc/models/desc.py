...
#set seed
    ...
    desc=DescModel(dims=dims,
              ...
              max_triplet_anchors_per_batch=max_triplet_anchors_per_batch,
              triplet_pseudo_label_method=triplet_pseudo_label_method
    )
    ...
    adata.uns.setdefault('desc_training_history', {})[str(louvain_resolution)] = {
        'loss_history': {
            'name': 'pretrain',
            'epochs': list(range(len(getattr(desc, 'pretrain_loss_history', [])))),
            'train_loss': list(getattr(desc, 'pretrain_loss_history', [])),
            'components': {
                'reconstruction': list(getattr(desc, 'pretrain_recon_history', [])),
                'rare_triplet': list(getattr(desc, 'pretrain_triplet_history', [])),
                'triplet_valid_anchors': list(
                    getattr(desc, 'pretrain_triplet_valid_anchor_history', [])
                ),
            },
        },
        'weight_history': list(getattr(desc, 'weight_history', [])),
    }
    return adata
