...
class DescModel(object):
    ...
    def pretrain(self):
		...
		act=self.activation,
                ...
                dynamic_weight_update_interval=self.dynamic_weight_update_interval,
                dynamic_weight_momentum=self.dynamic_weight_momentum,
                pseudo_label_method=self.pseudo_label_method,
                weight_exponent=self.weight_exponent,
                weight_n_clusters=self.weight_n_clusters,
                density_knn_k=self.density_knn_k,
                density_weight_exponent=self.density_weight_exponent,
                density_weight_clip=self.density_weight_clip,
                weight_fusion_mode=self.weight_fusion_mode,
                cluster_density_alpha=self.cluster_density_alpha,
                cluster_weight_power=self.cluster_weight_power,
                density_weight_power=self.density_weight_power,
                min_cell_weight=self.min_cell_weight,
                max_cell_weight=self.max_cell_weight,
                weight_component_mode=self.weight_component_mode,
                rare_triplet_weight=self.rare_triplet_weight,
                rare_triplet_margin=self.rare_triplet_margin,
                rare_triplet_min_weight=self.rare_triplet_min_weight,
                rare_triplet_start_epoch=self.rare_triplet_start_epoch,
                max_triplet_anchors_per_batch=self.max_triplet_anchors_per_batch,
                triplet_pseudo_label_method=self.triplet_pseudo_label_method
           )
        # begin pretraining
        t0 = get_time()
        ...
        self.sae = sae
        self.pretrain_loss_history = list(getattr(sae, "pretrain_loss_history", []))
        self.pretrain_recon_history = list(getattr(sae, "pretrain_recon_history", []))
        self.pretrain_triplet_history = list(getattr(sae, "pretrain_triplet_history", []))
        self.pretrain_triplet_valid_anchor_history = list(
            getattr(sae, "pretrain_triplet_valid_anchor_history", [])
        )
        self.weight_history = list(getattr(sae, "weight_history", []))
