...
class BaseAlgorithm(ABC):
    ...
    def get_weight_history(self) -> List[Dict[str, Any]]:
        """
        Get per-epoch cell-weight summary statistics.

        Returns:
            List of dicts with at least:
              - epoch: epoch number
              - mean/std/min/max: summary statistics of per-cell weights
            Optional keys such as phase, phase_epoch, p05, p50, p95, and
            refreshed are preserved when provided by an algorithm.
        """
        raw_history = getattr(self, "_weight_history", [])
        if not isinstance(raw_history, list):
            return []

        out: List[Dict[str, Any]] = []
        numeric_keys = ("mean", "std", "min", "p05", "p50", "p95", "max")
        int_keys = ("epoch", "phase_epoch")
            ...
            for key in int_keys:
                if key in raw:
                    try:
                        ...
                        pass
            if "phase" in raw:
                row["phase"] = str(raw["phase"])
            if "refreshed" in raw:
                row["refreshed"] = bool(raw["refreshed"])
