...
def _merge_metric_rows(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
...
def _read_final_metrics(run_dir: Path, requested: str, target_clusters: int) -> Dict[str, Any]:
    ...
    if method == "both_mean":
        available = []
        for candidate_method in ("hdbscan_final", f"leiden_target{int(target_clusters)}_final"):
            chosen = by_method.get(candidate_method)
        ...
        if not available:
            return {"used_method": "both_mean", "fallback": False}
        out = _merge_metric_rows(available)
        out["used_method"] = "both_mean"
        out["fallback"] = False
        return out
    ...
    if method == "both_max":
        ...
        for candidate_method in ("hdbscan_final", f"leiden_target{int(target_clusters)}_final"):
            ...
            candidates.append((_score(metrics), candidate_method, metrics))
        if not candidates:
            return {"used_method": "both_max", "fallback": False}
        _, used_method, best_metrics = max(candidates, key=lambda item: item[0])
        out = dict(best_metrics)
        out["used_method"] = used_method
        out["fallback"] = False
        return out
