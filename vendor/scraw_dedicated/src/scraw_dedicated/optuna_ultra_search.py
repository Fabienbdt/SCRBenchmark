...
def _selected_clustering_method(requested: str, target_clusters: int) -> str:
    ...
    if req in {"both_mean", "both_max"}:
        return req
    return "hdbscan_final"


def _merge_clustering_metrics(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Average finite metrics over several clustering outputs."""
    keys = (
        "ARI",
        "NMI",
        "ACC",
...
def _select_final_metrics(
    ...
    """Select or aggregate final clustering metrics for trial scoring."""
    req = str(requested).strip().lower()
    hdbscan_metrics = cluster_rows.get("hdbscan_final")
    leiden_method = f"leiden_target{int(target_clusters)}_final"
    leiden_metrics = cluster_rows.get(leiden_method)

    if req == "both_mean":
        available = [row for row in (hdbscan_metrics, leiden_metrics) if row is not None]
        if not available:
            return {}, "both_mean", 0
        metrics = _merge_clustering_metrics(available)
        metrics["used_method"] = "both_mean"
        return metrics, "both_mean", 0

    if req == "both_max":
        candidates: List[Tuple[float, str, Dict[str, Any]]] = []
        if hdbscan_metrics is not None:
            candidates.append((_composite_score(hdbscan_metrics), "hdbscan_final", hdbscan_metrics))
        if leiden_metrics is not None:
            candidates.append((_composite_score(leiden_metrics), leiden_method, leiden_metrics))
        if not candidates:
            return {}, "both_max", 0
        _, used_method, chosen_metrics = max(candidates, key=lambda item: item[0])
        metrics = dict(chosen_metrics)
        metrics["used_method"] = used_method
        return metrics, used_method, 0

    selected_method = _selected_clustering_method(requested, target_clusters)
    ...
    if metric_source is None and selected_method != "hdbscan_final":
        metric_source = hdbscan_metrics
        used_method = "hdbscan_final"
        if metric_source is not None:
            fallback_count = 1

    metrics = dict(metric_source) if metric_source is not None else {}
    metrics["used_method"] = used_method
    return metrics, used_method, fallback_count
...
def _build_cli_cmd(
    ...
    leiden_target_clusters: Optional[int],
    ...
    if leiden_target_clusters is not None and int(leiden_target_clusters) > 1:
        cmd.extend(["--leiden-target-clusters", str(int(leiden_target_clusters))])
...
def _sample_trial_config(trial: Any, args: argparse.Namespace) -> TrialConfig:
    ...
    cluster_density_alpha = trial.suggest_float("cluster_density_alpha", 0.0, 1.0)
    ...
    elif args.final_clustering_mode in {"both_mean", "both_max"}:
        final_clustering_requested = str(args.final_clustering_mode)
    else:
        final_clustering_requested = str(
    ...
    return {
        ...
        "spaces": {
            ...
            "dynamic_weighting": {
                ...
                "cluster_density_alpha": "0.0..1.0 continuous",
            ...
            "final_scoring_clustering": {
                "method": {
                    ...
                    "both_mean": (
                        "average of hdbscan_final and "
                        f"leiden_target{int(args.target_clusters)}_final scores"
                    ),
                    "both_max": (
                        "best of hdbscan_final and "
                        f"leiden_target{int(args.target_clusters)}_final scores"
                    ),
                },
                "leiden_resolution_search": "performed by scraw_dedicated.cli (0.0..1.0, step 0.05)",
            },
        },
    }
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        choices=["hdbscan", "leiden", "mixed", "both_mean", "both_max"],
        default="mixed",
        help="Final clustering method used for trial scoring; both_* scores both outputs per trial",
    )
    p.add_argument(
        "--target-clusters",
        type=int,
    ...
    def objective(trial: Any) -> float:
        ...
        for seed in seeds:
            ...
            cmd = _build_cli_cmd(
                ...
                seed=int(seed),
                batch_key=args.batch_key,
                leiden_target_clusters=int(args.target_clusters),
            ...
            metrics, used_method, seed_fallback_count = _select_final_metrics(
                cluster_rows=cluster_rows,
                requested=cfg.final_clustering_requested,
                target_clusters=int(args.target_clusters),
            )
            fallback_count += int(seed_fallback_count)
