...
from .metrics import NOISE_LABELS, align_labels
...
def compute_projection_2d(
    embeddings: np.ndarray,
    random_state: int = 42,
) -> Tuple[np.ndarray, bool]:
    """Compute one reusable 2D projection for a final embedding matrix."""
    return _compute_umap_or_2d(np.asarray(embeddings), random_state=random_state)


def _resolve_projection_2d(
    embeddings: np.ndarray,
    projection_2d: Optional[np.ndarray] = None,
    random_state: int = 42,
) -> Tuple[np.ndarray, bool]:
    """Reuse a provided 2D projection when available, otherwise fit one."""
    embeddings = np.asarray(embeddings)
    if projection_2d is None:
        return compute_projection_2d(embeddings, random_state=random_state)

    projection_arr = np.asarray(projection_2d)
    if projection_arr.ndim != 2 or projection_arr.shape[1] != 2:
        raise ValueError("projection_2d must be a 2D array with shape (n_cells, 2).")
    if projection_arr.shape[0] != embeddings.shape[0]:
        raise ValueError("projection_2d must have the same number of rows as embeddings.")
    return projection_arr, False


def _categorical_cmap(n_labels: int):
    """Return a categorical colormap sized for the number of labels."""
    if n_labels <= 10:
        return plt.cm.tab10
    if n_labels <= 20:
        return plt.cm.tab20
...
def format_cluster_labels_for_display(
    labels: np.ndarray,
    prefix: str = "cluster_",
) -> np.ndarray:
    """Format raw cluster IDs into stable human-readable labels."""
    out: List[str] = []
    for raw in np.asarray(labels, dtype=object):
        text = str(raw)
        if text in {str(x) for x in NOISE_LABELS}:
            out.append("noise")
            continue
        if text.startswith(prefix) or text.startswith("Unmatched_"):
            out.append(text)
            continue
...
def compute_projection_2d_per_snapshot(
    embeddings_per_snapshot: List[np.ndarray],
    random_state: int = 42,
) -> Tuple[List[np.ndarray], bool]:
    """Compute one 2D projection per snapshot for later figure reuse."""
    if not embeddings_per_snapshot:
        return [], True

    out: List[np.ndarray] = []
    used_any_fallback = False
    for emb in embeddings_per_snapshot:
        emb2d, used_fb = compute_projection_2d(np.asarray(emb), random_state=random_state)
        out.append(emb2d)
        used_any_fallback = used_any_fallback or bool(used_fb)
    return out, used_any_fallback
...
def _compute_cluster_label_anchor(points_2d: np.ndarray) -> np.ndarray:
    """Choose a label anchor on a dense, representative point of the cluster."""
    points = np.asarray(points_2d, dtype=float)
    if points.ndim != 2 or points.shape[0] == 0:
        raise ValueError("points_2d must contain at least one 2D point")
    if len(points) == 1:
        return points[0]

    robust_center = np.median(points, axis=0)
    if len(points) <= 8:
        idx = int(np.argmin(np.sum((points - robust_center) ** 2, axis=1)))
        return points[idx]

    try:
        from scipy.spatial import cKDTree

        k = min(max(5, int(np.sqrt(len(points)))), 24, len(points) - 1)
        tree = cKDTree(points)
        distances, _ = tree.query(points, k=k + 1)
        local_density_proxy = np.mean(distances[:, 1:], axis=1)
        center_distance = np.sqrt(np.sum((points - robust_center) ** 2, axis=1))

        density_scale = max(float(np.median(local_density_proxy)), 1e-6)
        center_scale = max(float(np.median(center_distance)), 1e-6)
        score = (local_density_proxy / density_scale) + 0.35 * (center_distance / center_scale)
...
def _relax_centroid_label_positions(
    ax: plt.Axes,
    text_artists: List[plt.Text],
    anchor_points: List[np.ndarray],
    iterations: int = 80,
) -> None:
    """Nudge centroid labels apart while keeping them close to their anchor points."""
    ...
    anchor_disp = np.asarray([ax.transData.transform(anchor) for anchor in anchor_points], dtype=float)
    xlim = ax.get_xlim()
    ylim = ax.get_ylim()
    ...
    for _ in range(iterations):
        ...
        bboxes = [text.get_window_extent(renderer=renderer).expanded(1.08, 1.18) for text in text_artists]
        centers = np.asarray(
            [[0.5 * (bbox.x0 + bbox.x1), 0.5 * (bbox.y0 + bbox.y1)] for bbox in bboxes],
            dtype=float,
        )
        ...
        for i in range(len(text_artists)):
            for j in range(i + 1, len(text_artists)):
                ...
                if overlap_x < overlap_y:
                    direction = np.array([1.0 if delta[0] >= 0 else -1.0, 0.0])
                    magnitude = overlap_x / 2.0 + 2.0
                else:
                    direction = np.array([0.0, 1.0 if delta[1] >= 0 else -1.0])
                    magnitude = overlap_y / 2.0 + 2.0

                push = direction * magnitude
                shifts[i] -= push
                shifts[j] += push

        # Keep labels tethered to a representative cluster point.
        shifts += 0.10 * (anchor_disp - centers)
        max_shift = float(np.max(np.linalg.norm(shifts, axis=1)))
        if not overlap_found and max_shift < 0.5:
            break

        for idx, text in enumerate(text_artists):
            current_disp = ax.transData.transform(text.get_position())
            proposed_disp = current_disp + np.clip(shifts[idx], -18.0, 18.0)
            proposed_data = inv.transform(proposed_disp)
            new_x = min(max(proposed_data[0], xlim[0] + pad_x), xlim[1] - pad_x)
            new_y = min(max(proposed_data[1], ylim[0] + pad_y), ylim[1] - pad_y)
...
def _draw_cluster_overlays(
    ...
    for idx, group_label in enumerate(unique_labels):
        ...
        if show_centroids:
            center = _compute_cluster_label_anchor(points)
            ...
            text = ax.text(
                ...
                color="black",
                path_effects=[
                    pe.withStroke(linewidth=3.0, foreground="white", alpha=0.95),
                ],
            )
            text_artists.append(text)
            anchor_points.append(center)

    # A dense in-cluster anchor is more reliable than a global centroid for
    # elongated / disconnected manifolds such as Paul15. We intentionally keep
    # the labels fixed on those anchors to avoid over-correcting them away from
    # their cluster.


def _style_umap_axes(ax: plt.Axes) -> None:
    """Remove axis visuals from UMAP plots while keeping titles and legends."""
    ax.grid(False)
    ax.set_xlabel("")
    ax.set_ylabel("")
    ax.set_xticks([])
...
def export_axes_panels(
    ...
    """Save each provided axis as an individual image."""
    if len(axes) != len(output_paths):
        raise ValueError("axes and output_paths must have the same length.")

    fig.canvas.draw()
    renderer = fig.canvas.get_renderer()
    if renderer is None:
        raise RuntimeError("Matplotlib renderer is not available for panel export.")

    dpi_to_inches = fig.dpi_scale_trans.inverted()
    for ax, raw_path in zip(axes, output_paths):
        bbox = ax.get_tightbbox(renderer)
        if bbox is None:
            continue
        output_path = Path(raw_path)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        bbox_inches = bbox.expanded(expand_x, expand_y).transformed(dpi_to_inches)
...
def plot_umap_comparison(
    ...
    dataset_info: Optional[str] = None,
    projection_2d: Optional[np.ndarray] = None,
    show_context_annotation: bool = True,
    ...
    raw_predicted_labels = format_cluster_labels_for_display(predicted_labels)
    try:
        aligned_predicted_labels = align_labels(true_labels, predicted_labels)
    except Exception:
        aligned_predicted_labels = predicted_labels

    aligned_predicted_labels = np.asarray(aligned_predicted_labels, dtype=object)
    aligned_predicted_labels = _tag_unmatched_predicted_labels(
        aligned_predicted_labels,
        label_names,
    )

    embeddings_2d, _ = _resolve_projection_2d(embeddings, projection_2d=projection_2d)

    true_str = np.asarray(true_labels, dtype=object).astype(str)
    raw_pred_str = np.asarray(raw_predicted_labels, dtype=object).astype(str)
    aligned_pred_str = np.asarray(aligned_predicted_labels, dtype=object).astype(str)

    true_unique = list(pd.unique(true_str))
    aligned_extra = [x for x in pd.unique(aligned_pred_str) if x not in true_unique]
    shared_unique: List[Any] = list(true_unique) + aligned_extra
    shared_map: Dict[Any, int] = {lbl: i for i, lbl in enumerate(shared_unique)}
    shared_cmap = _categorical_cmap(len(shared_unique))
    shared_norm = plt.Normalize(vmin=0, vmax=max(len(shared_unique) - 1, 1))

    raw_unique = list(pd.unique(raw_pred_str))
    raw_map: Dict[Any, int] = {lbl: i for i, lbl in enumerate(raw_unique)}
    raw_cmap = _categorical_cmap(len(raw_unique))
    raw_norm = plt.Normalize(vmin=0, vmax=max(len(raw_unique) - 1, 1))
    ...
    def _draw_panel(
        ...
        *,
        ...
        legend_label_names: Optional[Dict[int, str]] = None,
    ) -> None:
        labels_arr = np.asarray(labels, dtype=object).astype(str)
        encoded = np.array([label_to_idx.get(str(x), 0) for x in labels_arr])
        ax.scatter(
            embeddings_2d[:, 0],
            embeddings_2d[:, 1],
            c=encoded,
        ...
        if outline_mode != "none" or show_cluster_centroids:
            _draw_cluster_overlays(
                ...
                centroid_label_names=legend_label_names,
            )
        ...
        present_labels = [lbl for lbl in legend_labels if lbl in present]
        n_present = len(present_labels)
        if n_present <= 30:
            handles = []
            for lbl in present_labels:
                color_idx = label_to_idx.get(str(lbl), 0)
                handles.append(
                    plt.Line2D(
                        [0],
                        [0],
                        marker="o",
                        color="w",
                        markerfacecolor=cmap(norm(color_idx)),
                        markersize=7,
                        label=_decode_label_name(lbl, legend_label_names),
                    )
                )
            ax.legend(
                handles=handles,
    ...
    _draw_panel(
        ...
        cmap=shared_cmap,
        norm=shared_norm,
        label_to_idx=shared_map,
        legend_labels=shared_unique,
        legend_label_names=label_names,
    )
    _draw_panel(
        axes[2],
        aligned_pred_str,
        "Predicted (Hungarian aligned)",
        cmap=shared_cmap,
        norm=shared_norm,
        label_to_idx=shared_map,
        legend_labels=shared_unique,
    ...
    _draw_panel(
        axes[1],
        raw_pred_str,
        "Predicted Clusters (raw IDs)",
        cmap=raw_cmap,
        norm=raw_norm,
        label_to_idx=raw_map,
        legend_labels=raw_unique,
        legend_label_names=None,
    )

    fig.suptitle(f"{projection_name} Comparison: {algorithm_name}", fontsize=16, fontweight="bold", y=1.02)

    if show_context_annotation and (params_info or dataset_info):
...
def plot_umap_categorical(
    ...
    """Plot one UMAP colored by arbitrary categorical labels."""
    embeddings = np.asarray(embeddings)
    if embeddings.shape[0] == 0:
        return None

    embeddings_2d, _ = _resolve_projection_2d(
        embeddings,
    ...
    n_labels = len(unique_labels)
    cmap = _categorical_cmap(n_labels)
    norm = plt.Normalize(vmin=0, vmax=max(n_labels - 1, 1))
...
def plot_umap_snapshots_categorical_panels(
    ...
    if projection_2d_per_snapshot is not None:
        ...
        if len(umap_list) != len(valid):
            return None
    elif projection_mode == "per_snapshot":
        umap_list = []
        for snap in valid:
            emb2d, _ = _compute_umap_or_2d(
                np.asarray(snap["embeddings"]),
                random_state=random_state,
            )
            umap_list.append(emb2d)
    else:
...
def plot_umap_snapshots_gradient_panels(
    ...
    if projection_2d_per_snapshot is not None:
        ...
        umap_list = []
        for emb in arrays:
            emb2d, _ = _compute_umap_or_2d(emb, random_state=random_state)
            umap_list.append(emb2d)
    else:
    ...
    sm = ScalarMappable(cmap="plasma_r")
    sm.set_clim(0.0, 1.0)
...
def plot_metric_evolution_curves(
    ...
    for idx in range(len(metric_specs), n_rows * n_cols):
        axes[idx // n_cols][idx % n_cols].axis("off")
...
def plot_umap_weighted_gradient(
    ...
    """Plot UMAP with an inverted plasma gradient for reconstruction weights."""
    embeddings = np.asarray(embeddings)
    cell_weights = np.asarray(cell_weights, dtype=np.float64)
    if embeddings.shape[0] == 0:
        return None
