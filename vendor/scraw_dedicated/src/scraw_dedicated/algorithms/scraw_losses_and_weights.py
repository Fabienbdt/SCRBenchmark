...
def _to_numpy(x: Any, dtype: np.dtype | None = None) -> np.ndarray:
    ...
    # `AnnData` can contain sparse matrices; we densify them here
    # to simplify numerical operations in the rest of the pipeline.
...
class ScrawLossWeightMixin:
    ...
    def _transform_nb_input(
        ...
        # This function prepares the encoder input (and not the loss).
        ...
        if mode in {"identity", "none"}:
            # No change in count scale.
            return X_counts.copy()
        if mode == "log1p":
            # Standard single-cell transformation to compress large values.
        ...
        # Pearson residuals:
        # - mu: expected value under a simple marginal model,
        # - var: approximated NB variance,
    ...
    def _cluster_frequency_weights(self, pseudo_labels: np.ndarray) -> np.ndarray:
        ...
        if exp == 0.0:
            # Exponent zero => all cells have the same weight.
    ...
    def _density_weights(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        # Distance to the k-th neighbor: simple proxy for local density.
    ...
    def _combined_cell_weights_components(
        ...
        if fusion_mode == "multiplicative":
            # Mostly reinforces cells that are both rare and isolated.
        ...
        if not np.isfinite(mean_fused) or mean_fused <= 0.0:
            ...
            # Normalize to mean 1 to stabilize loss scale across epochs.
        ...
        # Final clipping: prevents a cell from dominating (or disappearing) in the loss.
    ...
    def _apply_random_mask(
        ...
        # Independent boolean mask per cell and per feature.
    ...
    def _reduce_per_sample_loss(
        ...
        if weight >= 1.0:
            # Extreme case: we only optimize the masked positions.
            denom = mask_f.sum(dim=1).clamp(min=1.0)
            return (per_elem_loss * mask_f).sum(dim=1) / denom
        if weight <= 0.0:
            # Extreme case: we ignore the masked / non-masked distinction.
            return per_elem_loss.mean(dim=1)

        # Intermediate case: weighted combination of the two areas.
    ...
    def _negative_binomial_loss_per_sample(
        ...
        # NB formula (negative log-likelihood) computed element-wise.
        ...
        # Safeguard: replace NaN/Inf to avoid breaking backpropagation.
    ...
    def _reconstruction_loss_per_sample(
        ...
        if mode == "nb":
            if size_factors is None:
                # Fallback branch if no size factors are provided.
    ...
    def _rare_triplet_loss(
        ...
        """Semi-hard triplet loss focused on high-weight (rare) anchors (vectorized).
        ...
        # Vectorized positive and negative mining
        B = z.shape[0]
        C = candidate.numel()
        
        # same_mask: (C, B) indicating cells of the same pseudo-cluster
        same_mask = labels.unsqueeze(0) == labels[candidate].unsqueeze(1)
        # Exclude anchor itself
        same_mask[torch.arange(C, device=z.device), candidate] = False
        
        diff_mask = ~same_mask
        diff_mask[torch.arange(C, device=z.device), candidate] = False

        # Keep only anchors that have at least one positive and one negative sample in the batch
        valid = same_mask.any(dim=1) & diff_mask.any(dim=1)
        if not valid.any():
            return torch.tensor(0.0, device=z.device)
        ...
        dists_valid = dists[candidate_valid] # (C_valid, B)
        
        # d_pos: farthest positive (max distance among cells with same label)
        d_pos = torch.where(same_mask, dists_valid, torch.tensor(-float('inf'), device=z.device)).max(dim=1).values
        
        # semi_hard_mask: negative samples with distance > d_pos
        semi_hard_mask = diff_mask & (dists_valid > d_pos.unsqueeze(1))
        
        d_neg_semi_hard = torch.where(semi_hard_mask, dists_valid, torch.tensor(float('inf'), device=z.device)).min(dim=1).values
        d_neg_fallback = torch.where(diff_mask, dists_valid, torch.tensor(float('inf'), device=z.device)).min(dim=1).values
        
        # Choose semi-hard negative if available, otherwise fallback to hardest negative overall
        d_neg = torch.where(semi_hard_mask.any(dim=1), d_neg_semi_hard, d_neg_fallback)
