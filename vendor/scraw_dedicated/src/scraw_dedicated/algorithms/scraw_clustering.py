"""Clustering and pseudo-label helpers used by scRAW (deep2-focused).

Two distinct clustering stages are implemented here:
1) pseudo-labeling during training (for dynamic weighting + triplet loss),
2) final clustering on the converged latent space (HDBSCAN + fallbacks).

This split is intentional: pseudo-labels are optimization scaffolding, while
final labels are the output exposed by the algorithm.
"""

from __future__ import annotations

from typing import Any, Dict, List, Optional, Tuple
import logging

import numpy as np

from ..defaults import DEFAULT_PARAM_OVERRIDES
from ..resolution_selection import (
    LEIDEN_RESOLUTION_GRID,
    compute_candidate_silhouette,
    select_best_leiden_candidate,
)


logger = logging.getLogger(__name__)


def remap_contiguous_labels(labels: np.ndarray) -> np.ndarray:
    """Remap arbitrary cluster ids to contiguous ids 0..K-1.

    Keeps ordering deterministic by sorting unique label values before remap.
    """
    labels = np.asarray(labels)
    uniq = sorted(np.unique(labels).tolist())
    mapping = {int(v): i for i, v in enumerate(uniq)}
    return np.asarray([mapping[int(v)] for v in labels], dtype=np.int64)


class ScrawClusteringMixin:
    """Pseudo-label + final clustering routines used by the deep2 workflow.

    This mixin deliberately isolates clustering heuristics from the main
    training loop so they can be audited or changed without touching the
    optimizer/reconstruction code path.
    """

    _pseudo_fallback_method: Optional[str]
    _leiden_warning_emitted: bool
    _dataset_n_classes: int
    _pseudo_clustering_info: Dict[str, Any]

    def _param(self, key: str, default: Any) -> Any:  # pragma: no cover
        """Read one algorithm parameter, delegating to the concrete parent class."""
        raise NotImplementedError

    def _clip_k_to_data(self, k: int, n_cells: int) -> int:
        """Clip k to valid bounds for the dataset size."""
        # Safety bound:
        # - K >= 2 (otherwise trivial clustering),
        # - K <= n_cells-1 to avoid degenerate cases.
        n_cells = int(max(1, n_cells))
        if n_cells == 1:
            return 1
        upper = 2 if n_cells == 2 else max(2, n_cells - 1)
        return int(max(2, min(int(k), upper)))

    def _estimate_unsupervised_k_heuristic(self, n_cells: int) -> int:
        """Legacy size-based heuristic to estimate a reasonable pseudo-label K."""
        n_cells = int(max(1, n_cells))
        k_min = int(max(2, self._param("unsupervised_k_min", 8)))
        k_max = int(max(2, self._param("unsupervised_k_max", 30)))
        if k_max < k_min:
            k_min, k_max = k_max, k_min

        k_est = int(round(float(np.sqrt(n_cells / 40.0))))
        k_est = int(np.clip(k_est, k_min, k_max))
        return self._clip_k_to_data(k_est, n_cells)

    def _build_unsupervised_k_candidates(self, n_cells: int) -> List[int]:
        """Build bounded candidate grid for stability-consensus K selection.

        If full `[k_min, k_max]` is too large, use an evenly spaced subset that
        still includes both bounds.
        """
        n_cells = int(max(1, n_cells))
        k_min = int(max(2, self._param("unsupervised_k_min", 8)))
        k_max = int(max(2, self._param("unsupervised_k_max", 30)))
        if k_max < k_min:
            k_min, k_max = k_max, k_min

        k_max = int(min(k_max, max(2, n_cells - 1)))
        k_min = int(min(k_min, k_max))
        if k_max <= 2:
            return [2]

        max_candidates = int(max(3, self._param("unsupervised_k_num_candidates", 12)))
        full_grid = list(range(k_min, k_max + 1))
        if len(full_grid) <= max_candidates:
            return full_grid

        # Uniform subsampling of the grid to limit computation cost.
        sampled = np.linspace(k_min, k_max, num=max_candidates)
        candidates = sorted(set(int(round(v)) for v in sampled))
        candidates = [k for k in candidates if 2 <= k <= k_max]
        if k_min not in candidates:
            candidates.insert(0, k_min)
        if k_max not in candidates:
            candidates.append(k_max)
        return sorted(set(candidates))

    def _prepare_unsupervised_k_embeddings(self, embeddings: np.ndarray) -> np.ndarray:
        """Prepare embeddings for K selection (optional PCA reduction)."""
        emb = np.asarray(embeddings, dtype=np.float32)
        if emb.ndim != 2 or emb.shape[0] <= 2:
            return emb

        pca_dim = int(self._param("unsupervised_k_pca_dim", 32))
        max_components = int(min(emb.shape[1], max(1, emb.shape[0] - 1)))
        if pca_dim <= 1 or max_components <= 2 or emb.shape[1] <= pca_dim:
            # If the reduction brings no benefit, we keep the original embedding.
            return emb

        from sklearn.decomposition import PCA

        n_components = int(max(2, min(pca_dim, max_components)))
        try:
            pca = PCA(
                n_components=n_components,
                svd_solver="randomized",
                random_state=int(self._param("random_state", self._param("seed", 42))),
            )
            return pca.fit_transform(emb).astype(np.float32, copy=False)
        except Exception:
            return emb

    def _fit_kmeans_for_k_selection(
        self,
        X: np.ndarray,
        n_clusters: int,
        random_state: int,
        n_init: int,
    ) -> np.ndarray:
        """Fit KMeans (or MiniBatchKMeans) for K-selection scoring."""
        from sklearn.cluster import KMeans, MiniBatchKMeans

        Xv = np.asarray(X, dtype=np.float32)
        n_samples = int(Xv.shape[0])
        n_clusters = int(max(2, min(int(n_clusters), n_samples)))
        n_init = int(max(1, n_init))

        if n_samples > 15000:
            # MiniBatchKMeans for large datasets: faster, slightly less precise.
            batch_size = int(min(2048, max(256, n_samples // 20)))
            model = MiniBatchKMeans(
                n_clusters=n_clusters,
                n_init=n_init,
                random_state=int(random_state),
                batch_size=batch_size,
                max_iter=200,
                reassignment_ratio=0.01,
            )
        else:
            model = KMeans(
                n_clusters=n_clusters,
                n_init=n_init,
                random_state=int(random_state),
            )
        labels = model.fit_predict(Xv)
        return np.asarray(labels, dtype=np.int64)

    def _rank_scores(self, metric_by_k: Dict[int, float], higher_is_better: bool) -> Dict[int, float]:
        """Convert metric values to [0,1] rank scores."""
        finite_items = [
            (int(k), float(v))
            for k, v in metric_by_k.items()
            if np.isfinite(float(v))
        ]
        if not finite_items:
            return {}

        ordered = sorted(finite_items, key=lambda kv: kv[1], reverse=bool(higher_is_better))
        n = len(ordered)
        if n == 1:
            return {ordered[0][0]: 1.0}
        # The best receives 1.0, the worst 0.0.
        return {
            int(k): float(1.0 - (idx / float(n - 1)))
            for idx, (k, _) in enumerate(ordered)
        }

    def _estimate_unsupervised_k_stability_consensus(
        self,
        embeddings: np.ndarray,
        n_cells: int,
    ) -> int:
        """Select K with weighted consensus of stability + internal CVIs.

        For each candidate K:
        - compute internal quality metrics (silhouette, CH, DB),
        - estimate run-to-run stability (mean pairwise ARI across repeated KMeans),
        - penalize excessive tiny clusters,
        - apply over/under-segmentation penalty versus a heuristic anchor K.

        Final score is a weighted rank-consensus, then highest score wins
        (tie-break: smallest K).
        """
        from sklearn.metrics import (
            adjusted_rand_score,
            calinski_harabasz_score,
            davies_bouldin_score,
            silhouette_score,
        )

        emb = self._prepare_unsupervised_k_embeddings(embeddings)
        n_samples = int(emb.shape[0])
        if n_samples <= 3:
            return self._clip_k_to_data(2, n_cells)

        candidates = self._build_unsupervised_k_candidates(n_samples)
        if not candidates:
            return self._estimate_unsupervised_k_heuristic(n_cells)

        random_state = int(self._param("random_state", self._param("seed", 42)))
        rng = np.random.RandomState(random_state)

        eval_sample_size = int(max(200, self._param("unsupervised_k_eval_sample_size", 3000)))
        stability_sample_size = int(
            max(300, self._param("unsupervised_k_stability_sample_size", 4000))
        )
        stability_runs = int(max(2, self._param("unsupervised_k_stability_runs", 5)))

        eval_n = int(min(n_samples, eval_sample_size))
        stab_n = int(min(n_samples, stability_sample_size))
        eval_idx = (
            rng.choice(n_samples, size=eval_n, replace=False)
            if eval_n < n_samples
            else np.arange(n_samples)
        )
        stab_idx = (
            rng.choice(n_samples, size=stab_n, replace=False)
            if stab_n < n_samples
            else np.arange(n_samples)
        )

        emb_eval = emb[eval_idx]
        emb_stab = emb[stab_idx]

        metric_sil: Dict[int, float] = {}
        metric_ch: Dict[int, float] = {}
        metric_db: Dict[int, float] = {}
        metric_stability: Dict[int, float] = {}
        metric_tiny_frac: Dict[int, float] = {}

        tiny_cluster_fraction = float(self._param("unsupervised_k_min_cluster_fraction", 0.005))
        tiny_cluster_fraction = float(np.clip(tiny_cluster_fraction, 0.0, 0.2))

        for k in candidates:
            if k >= n_samples:
                continue

            try:
                # Internal CVI metrics are evaluated on a fixed subsample for speed.
                labels_eval = self._fit_kmeans_for_k_selection(
                    X=emb_eval,
                    n_clusters=k,
                    random_state=random_state + (11 * int(k)),
                    n_init=10,
                )
                n_found = int(np.unique(labels_eval).size)
                if n_found <= 1 or n_found >= emb_eval.shape[0]:
                    continue
                counts = np.bincount(labels_eval.astype(np.int64, copy=False))
                tiny_abs = int(max(2, round(tiny_cluster_fraction * emb_eval.shape[0])))
                metric_tiny_frac[k] = float(np.mean(counts < tiny_abs))
                metric_sil[k] = float(silhouette_score(emb_eval, labels_eval, metric="euclidean"))
                metric_ch[k] = float(calinski_harabasz_score(emb_eval, labels_eval))
                metric_db[k] = float(davies_bouldin_score(emb_eval, labels_eval))
            except Exception:
                # If a K fails (numerical or convergence), we skip it.
                continue

            run_labels: List[np.ndarray] = []
            for run_id in range(stability_runs):
                try:
                    # Stability uses several KMeans runs with distinct seeds.
                    labels_run = self._fit_kmeans_for_k_selection(
                        X=emb_stab,
                        n_clusters=k,
                        random_state=random_state + 1009 + (run_id * 1291) + (17 * int(k)),
                        n_init=1,
                    )
                    if int(np.unique(labels_run).size) > 1:
                        run_labels.append(labels_run)
                except Exception:
                    continue

            if len(run_labels) >= 2:
                pairwise_ari: List[float] = []
                for i in range(len(run_labels)):
                    for j in range(i + 1, len(run_labels)):
                        pairwise_ari.append(float(adjusted_rand_score(run_labels[i], run_labels[j])))
                if pairwise_ari:
                    metric_stability[k] = float(np.mean(pairwise_ari))

        if not metric_sil:
            # No valid K: robust fallback on the simple heuristic.
            return self._estimate_unsupervised_k_heuristic(n_cells)

        score_stability = self._rank_scores(metric_stability, higher_is_better=True)
        score_sil = self._rank_scores(metric_sil, higher_is_better=True)
        score_ch = self._rank_scores(metric_ch, higher_is_better=True)
        score_db = self._rank_scores(metric_db, higher_is_better=False)
        score_tiny = self._rank_scores(metric_tiny_frac, higher_is_better=False)

        w_stability = max(0.0, float(self._param("unsupervised_k_weight_stability", 0.45)))
        w_sil = max(0.0, float(self._param("unsupervised_k_weight_silhouette", 0.25)))
        w_ch = max(0.0, float(self._param("unsupervised_k_weight_ch", 0.20)))
        w_db = max(0.0, float(self._param("unsupervised_k_weight_db", 0.10)))
        w_tiny = max(0.0, float(self._param("unsupervised_k_weight_tiny_clusters", 0.20)))
        overseg_penalty = max(0.0, float(self._param("unsupervised_k_overseg_penalty", 0.25)))
        underseg_penalty = max(0.0, float(self._param("unsupervised_k_underseg_penalty", 0.05)))
        k_anchor = self._estimate_unsupervised_k_heuristic(n_cells=n_cells)

        scored_k: Dict[int, float] = {}
        valid_ks = sorted(set(metric_sil.keys()) | set(metric_stability.keys()) | set(metric_ch.keys()) | set(metric_db.keys()))
        for k in valid_ks:
            # Weighted average of available rank scores for this K.
            weighted_sum = 0.0
            weight_sum = 0.0
            if k in score_stability:
                weighted_sum += w_stability * score_stability[k]
                weight_sum += w_stability
            if k in score_sil:
                weighted_sum += w_sil * score_sil[k]
                weight_sum += w_sil
            if k in score_ch:
                weighted_sum += w_ch * score_ch[k]
                weight_sum += w_ch
            if k in score_db:
                weighted_sum += w_db * score_db[k]
                weight_sum += w_db
            if k in score_tiny:
                weighted_sum += w_tiny * score_tiny[k]
                weight_sum += w_tiny

            if weight_sum <= 0:
                continue

            # Use a scale-free penalty around the anchor heuristic to avoid
            # pathological over/under-segmentation when metrics are noisy.
            raw_score = float(weighted_sum / weight_sum)
            over_ratio = max(0.0, (float(k) - float(k_anchor)) / max(1.0, float(k_anchor)))
            under_ratio = max(0.0, (float(k_anchor) - float(k)) / max(1.0, float(k_anchor)))
            penalty = float(overseg_penalty * over_ratio + underseg_penalty * under_ratio)
            scored_k[k] = float(raw_score - penalty)

        if not scored_k:
            return self._estimate_unsupervised_k_heuristic(n_cells)

        best_score = max(scored_k.values())
        tied = [k for k, s in scored_k.items() if abs(s - best_score) <= 1e-12]
        # Conservative tie-break: choose the smallest K.
        best_k = int(min(tied))
        return self._clip_k_to_data(best_k, n_cells)

    def _estimate_k(self, n_cells: int, embeddings: Optional[np.ndarray] = None) -> int:
        """Estimate pseudo-label K from user config, fallback, or automatic selection.

        Priority order:
        1) pre-resolved `_pseudo_n_clusters`,
        2) explicit `n_clusters`,
        3) manual unsupervised fallback,
        4) automatic unsupervised strategy (`heuristic` by default, or `stability_consensus`).
        """
        k_effective = int(self._param("_pseudo_n_clusters", 0) or 0)
        if k_effective > 1:
            # K already resolved upstream via explicit configuration.
            return self._clip_k_to_data(k_effective, n_cells)

        k_user = int(self._param("n_clusters", 0) or 0)
        if k_user > 1:
            # K explicitly set by the user.
            return self._clip_k_to_data(k_user, n_cells)

        manual_k = int(self._param("unsupervised_k_fallback", 0) or 0)
        if manual_k > 1:
            # Manual fallback takes priority over automatic strategies.
            k = self._clip_k_to_data(manual_k, n_cells)
            self.params["unsupervised_k_selected"] = int(k)
            self.params["unsupervised_k_selection_mode"] = "manual_fallback"
            return k

        mode = str(self._param("unsupervised_k_selection", "heuristic")).strip().lower()
        if mode not in {"stability_consensus", "heuristic"}:
            mode = "heuristic"

        if mode == "heuristic" or embeddings is None:
            # Without embedding (or forced mode), we cannot score stability/CVI.
            k = self._estimate_unsupervised_k_heuristic(n_cells)
            self.params["unsupervised_k_selected"] = int(k)
            self.params["unsupervised_k_selection_mode"] = (
                "heuristic_no_embeddings" if embeddings is None and mode != "heuristic" else "heuristic"
            )
            return k

        try:
            k = self._estimate_unsupervised_k_stability_consensus(embeddings=embeddings, n_cells=n_cells)
            self.params["unsupervised_k_selected"] = int(k)
            self.params["unsupervised_k_selection_mode"] = "stability_consensus"
            return self._clip_k_to_data(k, n_cells)
        except Exception:
            # Error in the consensus -> simple and traceable fallback.
            k = self._estimate_unsupervised_k_heuristic(n_cells)
            self.params["unsupervised_k_selected"] = int(k)
            self.params["unsupervised_k_selection_mode"] = "heuristic_after_failure"
            return k

    def _kmeans_pseudo_labels(self, embeddings: np.ndarray, k: Optional[int] = None) -> np.ndarray:
        """Compute KMeans pseudo-labels."""
        from sklearn.cluster import KMeans

        n_cells = embeddings.shape[0]
        k_eff = int(k) if k is not None else self._estimate_k(n_cells, embeddings=embeddings)
        k_eff = self._clip_k_to_data(k_eff, n_cells)

        km = KMeans(n_clusters=k_eff, random_state=int(self._param("seed", 42)), n_init=10)
        labels = km.fit_predict(embeddings)
        labels = remap_contiguous_labels(labels)
        self._record_pseudo_clustering_choice(
            {
                "method": "kmeans",
                "strategy": "fixed_k",
                "target_clusters": int(k_eff),
                "target_source": "pseudo_k",
                "requested_target_clusters": int(k_eff),
                "resolution": None,
                "n_clusters": int(np.unique(labels).size),
                "cluster_count_abs_error": int(abs(np.unique(labels).size - k_eff)),
                "selection_metric": "fixed_k",
                "selection_score": None,
            }
        )
        return labels

    def _pseudo_leiden_resolution_strategy(self) -> str:
        """Resolve the pseudo-Leiden resolution search strategy."""
        strategy = str(self._param("pseudo_leiden_resolution_strategy", "scraw_default")).strip().lower()
        if strategy not in {"scraw_default", "target_labels_silhouette"}:
            strategy = "scraw_default"
        return strategy

    def _resolve_pseudo_leiden_target_clusters(self, fallback_target_k: int, n_cells: int) -> Tuple[int, str]:
        """Resolve which cluster-count target the pseudo-Leiden search should use."""
        explicit_target = int(self._param("pseudo_leiden_target_clusters", 0) or 0)
        if explicit_target > 0:
            return self._clip_k_to_data(explicit_target, n_cells), "explicit_override"

        strategy = self._pseudo_leiden_resolution_strategy()
        dataset_n_classes = int(getattr(self, "_dataset_n_classes", 0) or 0)
        if strategy == "target_labels_silhouette" and dataset_n_classes > 0:
            return self._clip_k_to_data(dataset_n_classes, n_cells), "dataset_labels"

        return self._clip_k_to_data(fallback_target_k, n_cells), "pseudo_k"

    def _record_pseudo_clustering_choice(self, info: Dict[str, Any]) -> None:
        """Append one pseudo-clustering decision to the per-run audit trail."""
        payload = dict(getattr(self, "_pseudo_clustering_info", {}) or {})
        history = list(payload.get("history") or [])
        entry = dict(info)
        entry["call_index"] = int(len(history))
        history.append(entry)

        selected_resolutions: List[float] = []
        for item in history:
            try:
                resolution = float(item.get("resolution"))
            except Exception:
                continue
            if np.isfinite(resolution):
                selected_resolutions.append(resolution)

        payload["history"] = history
        payload["last"] = dict(entry)
        payload["method"] = str(entry.get("method", payload.get("method", "unknown")))
        payload["strategy"] = str(entry.get("strategy", payload.get("strategy", "")))
        payload["target_clusters"] = entry.get("target_clusters")
        payload["target_source"] = entry.get("target_source")
        payload["dataset_n_classes"] = int(getattr(self, "_dataset_n_classes", 0) or 0)
        payload["selected_resolutions"] = selected_resolutions
        self._pseudo_clustering_info = payload

    def _leiden_pseudo_labels(self, embeddings: np.ndarray, target_k: int) -> np.ndarray:
        """Compute Leiden pseudo-labels with resolution search toward target_k.

        Two strategies are supported:
        - `scraw_default`: mirrors the original scRAW fixed coarse grid.
        - `target_labels_silhouette`: scans 0..1 and breaks ties with silhouette.
        """
        import anndata as ad
        import scanpy as sc

        n_cells = embeddings.shape[0]
        strategy = self._pseudo_leiden_resolution_strategy()
        target_clusters, target_source = self._resolve_pseudo_leiden_target_clusters(target_k, n_cells)
        if n_cells < 3:
            self._record_pseudo_clustering_choice(
                {
                    "method": "leiden",
                    "strategy": strategy,
                    "target_clusters": int(target_clusters),
                    "target_source": str(target_source),
                    "requested_target_clusters": int(target_k),
                    "resolution": 0.0,
                    "n_clusters": 1,
                    "cluster_count_abs_error": int(abs(1 - int(target_clusters))),
                    "selection_metric": "degenerate_single_cluster",
                    "selection_score": None,
                }
            )
            return np.zeros(n_cells, dtype=np.int64)

        emb = np.asarray(embeddings, dtype=np.float32)
        adata = ad.AnnData(X=emb)
        rs = int(self._param("seed", 42))

        n_neighbors = min(15, n_cells - 1)
        # Neighbors graph on the latent embedding.
        sc.pp.neighbors(
            adata,
            n_neighbors=n_neighbors,
            use_rep="X",
            method="gauss",
            transformer="sklearn",
            random_state=rs,
        )

        if strategy == "target_labels_silhouette":
            candidates: List[Dict[str, Any]] = []
            for res in LEIDEN_RESOLUTION_GRID:
                sc.tl.leiden(adata, resolution=float(res), random_state=rs, key_added="_pseudo_leiden_tmp")
                labels = adata.obs["_pseudo_leiden_tmp"].astype(int).to_numpy(dtype=np.int64, copy=False)
                n_found = int(np.unique(labels).size)
                candidates.append(
                    {
                        "resolution": float(res),
                        "labels": labels.copy(),
                        "n_clusters": n_found,
                        "silhouette": compute_candidate_silhouette(
                            emb,
                            labels,
                            random_state=rs,
                        ),
                    }
                )

            best = select_best_leiden_candidate(candidates, expected_n_classes=int(target_clusters))
            if best is None:
                labels = np.zeros(n_cells, dtype=np.int64)
                self._record_pseudo_clustering_choice(
                    {
                        "method": "leiden",
                        "strategy": strategy,
                        "target_clusters": int(target_clusters),
                        "target_source": str(target_source),
                        "requested_target_clusters": int(target_k),
                        "resolution": None,
                        "n_clusters": 1,
                        "cluster_count_abs_error": int(abs(1 - int(target_clusters))),
                        "selection_metric": "cluster_count_abs_error_then_silhouette",
                        "selection_score": None,
                    }
                )
                return labels

            labels = remap_contiguous_labels(np.asarray(best["labels"], dtype=np.int64))
            n_found = int(np.unique(labels).size)
            silhouette = float(best.get("silhouette", float("nan")))
            self._record_pseudo_clustering_choice(
                {
                    "method": "leiden",
                    "strategy": strategy,
                    "target_clusters": int(target_clusters),
                    "target_source": str(target_source),
                    "requested_target_clusters": int(target_k),
                    "resolution": float(best["resolution"]),
                    "n_clusters": n_found,
                    "cluster_count_abs_error": int(abs(n_found - int(target_clusters))),
                    "selection_metric": "cluster_count_abs_error_then_silhouette",
                    "selection_score": silhouette if np.isfinite(silhouette) else None,
                    "silhouette": silhouette if np.isfinite(silhouette) else None,
                }
            )
            return labels

        k_eff = int(target_clusters)
        best_res, best_diff = 1.0, n_cells
        for res in np.arange(0.05, 3.0, 0.05):
            sc.tl.leiden(adata, resolution=float(res), random_state=rs)
            n_found = int(np.unique(adata.obs["leiden"].astype(int).values).size)
            diff = abs(n_found - k_eff)
            if diff < best_diff:
                best_diff = diff
                best_res = float(res)
            if n_found == k_eff:
                break

        sc.tl.leiden(adata, resolution=best_res, random_state=rs)
        labels = remap_contiguous_labels(adata.obs["leiden"].astype(int).values)
        n_found = int(np.unique(labels).size)
        self._record_pseudo_clustering_choice(
            {
                "method": "leiden",
                "strategy": strategy,
                "target_clusters": int(target_clusters),
                "target_source": str(target_source),
                "requested_target_clusters": int(target_k),
                "resolution": float(best_res),
                "n_clusters": n_found,
                "cluster_count_abs_error": int(abs(n_found - int(target_clusters))),
                "selection_metric": "cluster_count_abs_error",
                "selection_score": None,
            }
        )
        return labels

    def _pseudo_labels(self, embeddings: np.ndarray) -> np.ndarray:
        """Compute pseudo-labels used for dynamic weighting and triplet loss.

        If Leiden fails once, fallback is persisted to KMeans for the rest of
        the run to avoid repeated expensive failures inside training loops.
        """
        embeddings = self._sanitize_embeddings(embeddings)
        method = str(self._param("pseudo_label_method", "leiden")).strip().lower()
        if self._pseudo_fallback_method is not None:
            # "Sticky" fallback: once Leiden fails, we stick to KMeans.
            method = self._pseudo_fallback_method
        if method not in {"leiden", "kmeans"}:
            method = "kmeans"

        k = self._estimate_k(embeddings.shape[0], embeddings=embeddings)
        if method == "kmeans":
            return self._kmeans_pseudo_labels(embeddings, k=k)

        try:
            return self._leiden_pseudo_labels(embeddings, target_k=k)
        except Exception as exc:
            if not self._leiden_warning_emitted:
                logger.warning(
                    "pseudo_label_method=leiden failed once (%s); falling back to KMeans for the rest of the run.",
                    exc,
                )
                self._leiden_warning_emitted = True
            self._pseudo_fallback_method = "kmeans"
            return self._kmeans_pseudo_labels(embeddings, k=k)

    @staticmethod
    def _sanitize_embeddings(values: np.ndarray) -> np.ndarray:
        """Ensure finite values before distance or clustering ops."""
        arr = np.asarray(values, dtype=np.float32)
        if arr.size == 0:
            return arr
        non_finite = ~np.isfinite(arr)
        if np.any(non_finite):
            # Non-finite values replaced before distance calculation.
            arr = np.nan_to_num(arr, nan=0.0, posinf=1e4, neginf=-1e4)
        arr = np.clip(arr, -1e4, 1e4).astype(np.float32, copy=False)
        return arr

    def _estimate_final_target_k_unsupervised(self, embeddings: np.ndarray) -> Tuple[int, str]:
        """Estimate the final-clustering target K without using ground-truth labels."""
        emb = self._sanitize_embeddings(embeddings)
        n_cells = int(max(1, emb.shape[0]))

        final_target_k = int(self._param("final_target_k", 0) or 0)
        if final_target_k > 1:
            return self._clip_k_to_data(final_target_k, n_cells), "final_target_k"

        k_user = int(self._param("n_clusters", 0) or 0)
        if k_user > 1:
            return self._clip_k_to_data(k_user, n_cells), "n_clusters"

        manual_k = int(self._param("unsupervised_k_fallback", 0) or 0)
        if manual_k > 1:
            return self._clip_k_to_data(manual_k, n_cells), "unsupervised_k_fallback"

        mode = str(self._param("unsupervised_k_selection", "heuristic")).strip().lower()
        if mode not in {"stability_consensus", "heuristic"}:
            mode = "heuristic"

        if mode == "heuristic" or n_cells <= 3:
            return self._estimate_unsupervised_k_heuristic(n_cells), "heuristic"

        try:
            k = self._estimate_unsupervised_k_stability_consensus(embeddings=emb, n_cells=n_cells)
            return self._clip_k_to_data(k, n_cells), "stability_consensus"
        except Exception:
            return self._estimate_unsupervised_k_heuristic(n_cells), "heuristic_after_failure"

    def _build_final_hdbscan_scan_grid(
        self,
        n_cells: int,
        base_min_cluster_size: int,
        base_min_samples: int,
        base_method: str,
    ) -> List[Dict[str, Any]]:
        """Build a compact HDBSCAN grid for the final unsupervised scan."""
        n_cells = int(max(1, n_cells))
        max_mcs = int(max(2, min(n_cells - 1, max(20, int(np.ceil(0.02 * n_cells))))))

        size_candidates = [
            2,
            4,
            int(base_min_cluster_size),
            int(np.ceil(0.0025 * n_cells)),
            int(np.ceil(0.0050 * n_cells)),
            int(np.ceil(0.0100 * n_cells)),
            int(np.ceil(0.0200 * n_cells)),
        ]
        size_candidates = sorted(
            {
                int(np.clip(v, 2, max_mcs))
                for v in size_candidates
                if int(v) >= 2
            }
        )

        sample_candidates = sorted(
            {
                1,
                2,
                4,
                8,
                int(max(1, base_min_samples)),
            }
        )

        methods = [str(base_method)]
        if bool(self._param("hdbscan_scan_include_alternative_method", True)):
            alt = "leaf" if str(base_method) == "eom" else "eom"
            methods.append(alt)
        methods = [m for m in dict.fromkeys(methods) if m in {"eom", "leaf"}]
        if not methods:
            methods = ["eom"]

        grid: List[Dict[str, Any]] = []
        seen = set()
        for min_cluster_size in size_candidates:
            for min_samples in sample_candidates:
                spec = (
                    int(min_cluster_size),
                    int(min(min_samples, min_cluster_size)),
                    str(methods[0]),
                )
                if spec not in seen:
                    seen.add(spec)
                    grid.append(
                        {
                            "min_cluster_size": int(spec[0]),
                            "min_samples": int(spec[1]),
                            "cluster_selection_method": str(spec[2]),
                        }
                    )
            for method in methods[1:]:
                for min_samples in sample_candidates:
                    spec = (
                        int(min_cluster_size),
                        int(min(min_samples, min_cluster_size)),
                        str(method),
                    )
                    if spec in seen:
                        continue
                    seen.add(spec)
                    grid.append(
                        {
                            "min_cluster_size": int(spec[0]),
                            "min_samples": int(spec[1]),
                            "cluster_selection_method": str(spec[2]),
                        }
                    )
        return grid

    def _cluster_stratified_sample_indices(
        self,
        labels: np.ndarray,
        max_size: int,
        random_state: int,
    ) -> np.ndarray:
        """Subsample while keeping every cluster represented for silhouette scoring."""
        labs = np.asarray(labels, dtype=np.int64)
        n = int(labs.shape[0])
        max_size = int(max(2, max_size))
        if n <= max_size:
            return np.arange(n, dtype=np.int64)

        uniq = np.unique(labs)
        rng = np.random.RandomState(int(random_state))
        min_per_cluster = max(4, int(max_size // max(10, 2 * len(uniq))))

        picked: List[int] = []
        leftovers: List[np.ndarray] = []
        for clu in uniq:
            idx = np.where(labs == int(clu))[0]
            if idx.size <= min_per_cluster:
                picked.extend(idx.tolist())
                continue
            take = rng.choice(idx, size=min_per_cluster, replace=False)
            picked.extend(take.tolist())
            leftovers.append(np.setdiff1d(idx, take, assume_unique=False))

        if len(picked) >= max_size:
            picked = rng.choice(np.asarray(picked, dtype=np.int64), size=max_size, replace=False).tolist()
            return np.sort(np.asarray(picked, dtype=np.int64))

        budget = int(max_size - len(picked))
        if budget > 0 and leftovers:
            pool = np.concatenate([arr for arr in leftovers if arr.size > 0], axis=0)
            if pool.size > 0:
                if pool.size > budget:
                    extra = rng.choice(pool, size=budget, replace=False)
                else:
                    extra = pool
                picked.extend(np.asarray(extra, dtype=np.int64).tolist())

        return np.sort(np.asarray(picked[:max_size], dtype=np.int64))

    def _rare_weighted_silhouette_proxy(
        self,
        embeddings: np.ndarray,
        labels: np.ndarray,
    ) -> Dict[str, Any]:
        """Compute an unsupervised silhouette proxy that emphasizes rare clusters."""
        from sklearn.metrics import silhouette_samples

        emb = self._sanitize_embeddings(embeddings)
        labs = np.asarray(labels, dtype=np.int64)
        uniq = np.unique(labs)
        if emb.ndim != 2 or emb.shape[0] != labs.shape[0] or uniq.size < 2:
            return {
                "silhouette": float("nan"),
                "rare_weighted_silhouette": float("nan"),
                "n_eval_cells": 0,
            }

        eval_size = int(max(200, self._param("hdbscan_scan_eval_sample_size", 3000)))
        idx = self._cluster_stratified_sample_indices(
            labs,
            max_size=eval_size,
            random_state=int(self._param("random_state", self._param("seed", 42))),
        )
        emb_eval = emb[idx]
        labs_eval = labs[idx]
        if np.unique(labs_eval).size < 2:
            return {
                "silhouette": float("nan"),
                "rare_weighted_silhouette": float("nan"),
                "n_eval_cells": int(len(idx)),
            }

        try:
            sil_samples = np.asarray(
                silhouette_samples(emb_eval, labs_eval, metric="euclidean"),
                dtype=np.float32,
            )
        except Exception:
            return {
                "silhouette": float("nan"),
                "rare_weighted_silhouette": float("nan"),
                "n_eval_cells": int(len(idx)),
            }

        weights_full = np.asarray(self._cluster_frequency_weights(labs), dtype=np.float32)
        weights_eval = np.asarray(weights_full[idx], dtype=np.float32)
        weights_sum = float(np.sum(weights_eval))
        if not np.isfinite(weights_sum) or weights_sum <= 0.0:
            weights_eval = np.ones_like(weights_eval, dtype=np.float32)
            weights_sum = float(np.sum(weights_eval))

        rare_sil = float(np.sum(weights_eval * sil_samples) / max(weights_sum, 1e-8))
        plain_sil = float(np.mean(sil_samples))
        return {
            "silhouette": plain_sil,
            "rare_weighted_silhouette": rare_sil,
            "n_eval_cells": int(len(idx)),
        }

    def _run_hdbscan_raw(
        self,
        embeddings: np.ndarray,
        *,
        min_cluster_size: int,
        min_samples: int,
        cluster_selection_method: str,
    ) -> np.ndarray:
        """Run one raw HDBSCAN fit without fallbacks or post-processing."""
        import hdbscan as hdbscan_lib

        clusterer = hdbscan_lib.HDBSCAN(
            min_cluster_size=int(min_cluster_size),
            min_samples=int(min_samples),
            cluster_selection_method=str(cluster_selection_method),
            metric="euclidean",
            core_dist_n_jobs=1,
        )
        return np.asarray(clusterer.fit_predict(embeddings), dtype=np.int64)

    def _postprocess_hdbscan_labels(
        self,
        embeddings: np.ndarray,
        labels: np.ndarray,
        *,
        reassign_noise: bool,
        fallback_to_kmeans_on_all_noise: bool,
    ) -> np.ndarray:
        """Apply the standard post-processing stack to HDBSCAN labels."""
        labs = np.asarray(labels, dtype=np.int64).copy()

        if reassign_noise and np.any(labs < 0):
            labs = self._reassign_noise_to_centroids(
                embeddings,
                labs,
                fallback_to_kmeans=fallback_to_kmeans_on_all_noise,
            )

        if np.any(labs < 0):
            if np.any(labs >= 0):
                labs[labs < 0] = int(labs[labs >= 0].max()) + 1
            else:
                return np.asarray(labs, dtype=np.int64)

        labs = remap_contiguous_labels(labs)
        labs = self._merge_close_hdbscan_siblings(embeddings, labs)
        return remap_contiguous_labels(labs)

    def _select_hdbscan_candidate_unsupervised(
        self,
        embeddings: np.ndarray,
        *,
        min_cluster_size: int,
        min_samples: int,
        cluster_selection_method: str,
        reassign_noise: bool,
        target_clusters: int,
    ) -> Optional[Dict[str, Any]]:
        """Evaluate one HDBSCAN candidate with a rare-cell-oriented unsupervised score."""
        try:
            raw_labels = self._run_hdbscan_raw(
                embeddings,
                min_cluster_size=min_cluster_size,
                min_samples=min_samples,
                cluster_selection_method=cluster_selection_method,
            )
        except Exception:
            return None

        noise_fraction = float(np.mean(raw_labels < 0)) if raw_labels.size else 0.0
        labels = self._postprocess_hdbscan_labels(
            embeddings,
            raw_labels,
            reassign_noise=reassign_noise,
            fallback_to_kmeans_on_all_noise=False,
        )
        n_found = int(np.sum(np.unique(labels) >= 0))
        if n_found <= 1:
            return None

        proxy = self._rare_weighted_silhouette_proxy(embeddings, labels)
        rare_sil = float(proxy.get("rare_weighted_silhouette", float("nan")))
        plain_sil = float(proxy.get("silhouette", float("nan")))
        if not np.isfinite(rare_sil):
            return None

        diff = int(abs(int(n_found) - int(target_clusters)))
        diff_ratio = float(diff) / max(1.0, float(target_clusters))
        selection_score = float((0.8 * rare_sil) + (0.2 * plain_sil) - (0.15 * diff_ratio) - (0.10 * noise_fraction))

        return {
            "labels": np.asarray(labels, dtype=np.int64),
            "min_cluster_size": int(min_cluster_size),
            "min_samples": int(min_samples),
            "cluster_selection_method": str(cluster_selection_method),
            "n_clusters": int(n_found),
            "cluster_diff": int(diff),
            "noise_fraction": float(noise_fraction),
            "silhouette": float(plain_sil),
            "rare_weighted_silhouette": float(rare_sil),
            "n_eval_cells": int(proxy.get("n_eval_cells", 0)),
            "selection_score": float(selection_score),
        }

    def _final_hdbscan_scan(self, embeddings: np.ndarray) -> Tuple[Optional[np.ndarray], Dict[str, Any]]:
        """Scan a small HDBSCAN grid using an unsupervised rare-cell-oriented score."""
        emb = self._sanitize_embeddings(embeddings)
        n_cells = int(emb.shape[0])
        if n_cells < 3:
            return None, {}

        target_clusters, target_source = self._estimate_final_target_k_unsupervised(emb)
        min_cluster_size = max(
            2,
            int(
                self._param(
                    "hdbscan_min_cluster_size",
                    DEFAULT_PARAM_OVERRIDES["hdbscan_min_cluster_size"],
                )
                or 2
            ),
        )
        min_samples = max(
            1,
            int(
                self._param(
                    "hdbscan_min_samples",
                    DEFAULT_PARAM_OVERRIDES["hdbscan_min_samples"],
                )
                or 1
            ),
        )
        min_samples = min(min_samples, min_cluster_size)
        method = str(self._param("hdbscan_cluster_selection_method", "eom")).strip().lower()
        if method not in {"eom", "leaf"}:
            method = "eom"
        reassign_noise = bool(self._param("hdbscan_reassign_noise", True))

        candidates = self._build_final_hdbscan_scan_grid(
            n_cells=n_cells,
            base_min_cluster_size=min_cluster_size,
            base_min_samples=min_samples,
            base_method=method,
        )

        candidate_rows: List[Dict[str, Any]] = []
        valid: List[Dict[str, Any]] = []
        for spec in candidates:
            result = self._select_hdbscan_candidate_unsupervised(
                emb,
                min_cluster_size=int(spec["min_cluster_size"]),
                min_samples=int(spec["min_samples"]),
                cluster_selection_method=str(spec["cluster_selection_method"]),
                reassign_noise=reassign_noise,
                target_clusters=target_clusters,
            )
            if result is None:
                candidate_rows.append(
                    {
                        "min_cluster_size": int(spec["min_cluster_size"]),
                        "min_samples": int(spec["min_samples"]),
                        "cluster_selection_method": str(spec["cluster_selection_method"]),
                        "valid": False,
                    }
                )
                continue
            valid.append(result)
            candidate_rows.append(
                {
                    "min_cluster_size": int(result["min_cluster_size"]),
                    "min_samples": int(result["min_samples"]),
                    "cluster_selection_method": str(result["cluster_selection_method"]),
                    "n_clusters": int(result["n_clusters"]),
                    "cluster_diff": int(result["cluster_diff"]),
                    "noise_fraction": float(result["noise_fraction"]),
                    "silhouette": float(result["silhouette"]),
                    "rare_weighted_silhouette": float(result["rare_weighted_silhouette"]),
                    "selection_score": float(result["selection_score"]),
                    "valid": True,
                }
            )

        if not valid:
            return None, {
                "method": "hdbscan",
                "scan_enabled": True,
                "selection_metric": "rare_weighted_silhouette_unsupervised",
                "target_clusters": int(target_clusters),
                "target_source": str(target_source),
                "candidate_results": candidate_rows,
            }

        best = sorted(
            valid,
            key=lambda r: (
                -float(r["selection_score"]),
                -float(r["rare_weighted_silhouette"]),
                int(r["cluster_diff"]),
                -float(r["silhouette"]),
                float(r["noise_fraction"]),
                int(r["min_cluster_size"]),
                int(r["min_samples"]),
                0 if str(r["cluster_selection_method"]) == method else 1,
            ),
        )[0]

        info = {
            "method": "hdbscan",
            "scan_enabled": True,
            "selection_metric": "rare_weighted_silhouette_unsupervised",
            "selection_score": float(best["selection_score"]),
            "rare_weighted_silhouette": float(best["rare_weighted_silhouette"]),
            "silhouette": float(best["silhouette"]),
            "target_clusters": int(target_clusters),
            "target_source": str(target_source),
            "n_clusters": int(best["n_clusters"]),
            "cluster_diff": int(best["cluster_diff"]),
            "noise_fraction": float(best["noise_fraction"]),
            "min_cluster_size": int(best["min_cluster_size"]),
            "min_samples": int(best["min_samples"]),
            "cluster_selection_method": str(best["cluster_selection_method"]),
            "n_eval_cells": int(best["n_eval_cells"]),
            "candidate_results": candidate_rows,
        }
        return np.asarray(best["labels"], dtype=np.int64), info

    def _hdbscan_clustering(self, embeddings: np.ndarray) -> np.ndarray:
        """Apply final HDBSCAN clustering on latent embeddings.

        Safety pipeline:
        - sanitize embeddings,
        - run HDBSCAN,
        - fallback to Leiden/KMeans on failure or degenerate partition,
        - optionally reassign noise points,
        - remap labels to contiguous ids.
        """
        min_cluster_size = max(
            2,
            int(
                self._param(
                    "hdbscan_min_cluster_size",
                    DEFAULT_PARAM_OVERRIDES["hdbscan_min_cluster_size"],
                )
                or 2
            ),
        )
        min_samples = max(
            1,
            int(
                self._param(
                    "hdbscan_min_samples",
                    DEFAULT_PARAM_OVERRIDES["hdbscan_min_samples"],
                )
                or 1
            ),
        )
        if min_samples > min_cluster_size:
            logger.warning(
                "HDBSCAN: min_samples (%d) > min_cluster_size (%d). Clamping to %d.",
                min_samples,
                min_cluster_size,
                min_cluster_size,
            )
            min_samples = min_cluster_size

        method = str(self._param("hdbscan_cluster_selection_method", "eom")).strip().lower()
        if method not in {"eom", "leaf"}:
            logger.warning(
                "HDBSCAN: invalid cluster_selection_method='%s'. Falling back to 'eom'.",
                method,
            )
            method = "eom"
        reassign_noise = bool(self._param("hdbscan_reassign_noise", True))

        emb = self._sanitize_embeddings(embeddings)
        # Fallback K used only if HDBSCAN fails.
        k_fallback = self._estimate_k(emb.shape[0], embeddings=emb)

        def _fallback_to_leiden(reason: str) -> np.ndarray:
            """Fallback path when HDBSCAN fails or returns a degenerate partition."""
            logger.warning("HDBSCAN fallback to Leiden (%s).", reason)
            try:
                labels_fb = self._leiden_pseudo_labels(emb, target_k=k_fallback)
                self._final_clustering_info = {
                    "method": "leiden_fallback",
                    "scan_enabled": False,
                    "selection_metric": "rare_weighted_silhouette_unsupervised",
                    "fallback_reason": str(reason),
                    "target_clusters": int(k_fallback),
                }
                return remap_contiguous_labels(labels_fb)
            except Exception as exc:
                logger.warning("Leiden fallback failed (%s); fallback to KMeans.", exc)
                self._final_clustering_info = {
                    "method": "kmeans_fallback",
                    "scan_enabled": False,
                    "selection_metric": "rare_weighted_silhouette_unsupervised",
                    "fallback_reason": str(reason),
                    "target_clusters": int(k_fallback),
                }
                return self._kmeans_pseudo_labels(emb, k=k_fallback)

        try:
            labels = self._run_hdbscan_raw(
                emb,
                min_cluster_size=min_cluster_size,
                min_samples=min_samples,
                cluster_selection_method=method,
            )
        except Exception as exc:
            return _fallback_to_leiden(str(exc))

        n_found = int(np.sum(np.unique(labels) >= 0))
        if n_found <= 1:
            # Unusable final clustering: we force a usable fallback.
            return _fallback_to_leiden(f"degenerate result (n_clusters={n_found})")

        noise_fraction = float(np.mean(labels < 0)) if labels.size else 0.0
        labels = self._postprocess_hdbscan_labels(
            emb,
            labels,
            reassign_noise=reassign_noise,
            fallback_to_kmeans_on_all_noise=True,
        )
        labels = remap_contiguous_labels(labels)
        proxy = self._rare_weighted_silhouette_proxy(emb, labels)
        self._final_clustering_info = {
            "method": "hdbscan",
            "scan_enabled": False,
            "selection_metric": "rare_weighted_silhouette_unsupervised",
            "selection_score": float(proxy.get("rare_weighted_silhouette", float("nan"))),
            "rare_weighted_silhouette": float(proxy.get("rare_weighted_silhouette", float("nan"))),
            "silhouette": float(proxy.get("silhouette", float("nan"))),
            "target_clusters": int(k_fallback),
            "target_source": "legacy_estimate_k",
            "n_clusters": int(len(np.unique(labels))),
            "cluster_diff": int(abs(len(np.unique(labels)) - int(k_fallback))),
            "noise_fraction": float(noise_fraction),
            "min_cluster_size": int(min_cluster_size),
            "min_samples": int(min_samples),
            "cluster_selection_method": str(method),
            "n_eval_cells": int(proxy.get("n_eval_cells", 0)),
            "candidate_results": [],
        }
        return labels

    def _reassign_noise_to_centroids(
        self,
        embeddings: np.ndarray,
        labels: np.ndarray,
        *,
        fallback_to_kmeans: bool = True,
    ) -> np.ndarray:
        """Reassign HDBSCAN noise points to nearest non-noise centroid.

        If all points are noise, fallback directly to KMeans pseudo-labels.
        """
        labels = np.asarray(labels, dtype=np.int64).copy()
        keep = labels >= 0
        if not np.any(keep):
            if fallback_to_kmeans:
                from sklearn.cluster import KMeans

                k_fb = max(2, min(int(embeddings.shape[0]) - 1, 8))
                labels_fb = KMeans(n_clusters=k_fb, random_state=42, n_init=10).fit_predict(embeddings)
                return remap_contiguous_labels(labels_fb)
            return labels

        uniq = sorted(np.unique(labels[keep]).tolist())
        centroids = np.asarray(
            [np.mean(embeddings[labels == c], axis=0) for c in uniq], dtype=np.float32
        )

        noise_idx = np.where(labels < 0)[0]
        for i in noise_idx:
            # Reassignment by nearest centroid (Euclidean distance).
            d = np.sum((centroids - embeddings[i]) ** 2, axis=1)
            labels[i] = int(uniq[int(np.argmin(d))])
        return labels

    def _merge_close_hdbscan_siblings(self, embeddings: np.ndarray, labels: np.ndarray) -> np.ndarray:
        """Merge one obviously over-segmented close cluster pair (if any).

        Disabled by default. This post-processing is conservative and merges
        at most one pair to limit accidental topology distortion.
        """
        # Disabled by default to keep a conservative post-processing behavior.
        enable = bool(self._param("hdbscan_merge_close_clusters", False))
        if not enable:
            return np.asarray(labels, dtype=np.int64)

        labs = np.asarray(labels, dtype=np.int64).copy()
        uniq = sorted(np.unique(labs).tolist())
        if len(uniq) < 2:
            return labs

        min_size = int(self._param("hdbscan_merge_min_cluster_size", 200) or 200)
        max_ratio = float(self._param("hdbscan_merge_max_centroid_ratio", 1.15) or 1.15)
        if max_ratio <= 0.0:
            return labs

        sizes = {int(c): int(np.sum(labs == c)) for c in uniq}
        eligible = [int(c) for c in uniq if sizes[int(c)] >= max(1, min_size)]
        if len(eligible) < 2:
            return labs

        centroids: Dict[int, np.ndarray] = {}
        radii: Dict[int, float] = {}
        for c in eligible:
            pts = embeddings[labs == c]
            if pts.shape[0] == 0:
                continue
            cent = np.mean(pts, axis=0).astype(np.float32, copy=False)
            d = np.sqrt(np.sum((pts - cent) ** 2, axis=1))
            centroids[c] = cent
            radii[c] = float(np.median(d)) + 1e-8

        keys = sorted(centroids.keys())
        if len(keys) < 2:
            return labs

        best_pair: Optional[tuple[int, int]] = None
        best_ratio = np.inf
        for i, ca in enumerate(keys):
            for cb in keys[i + 1 :]:
                d_ab = float(np.sqrt(np.sum((centroids[ca] - centroids[cb]) ** 2)))
                # Ratio < 1 => centroids are close relative to internal dispersion.
                ratio = d_ab / (radii[ca] + radii[cb])
                if ratio < best_ratio:
                    best_ratio = ratio
                    best_pair = (int(ca), int(cb))

        if best_pair is None or not np.isfinite(best_ratio) or best_ratio > max_ratio:
            return labs

        a, b = best_pair
        logger.info(
            "HDBSCAN sibling merge applied: merge %d <- %d (ratio=%.4f, min_size=%d).",
            a,
            b,
            best_ratio,
            min_size,
        )
        labs[labs == b] = a
        return labs
