...
class ScRAWAlgorithm(BaseAutoencoderAlgorithm, ScrawLossWeightMixin, ScrawClusteringMixin):
    ...
    def get_hyperparameters(cls) -> List[HyperparameterConfig]:
        ...
        hp.extend(
            [
                ...
                HyperparameterConfig(
                    name="final_target_k",
                    display_name="Final Target K",
                    param_type=ParamType.INTEGER,
                    default=0,
                    min_value=0,
                    max_value=1000,
                    description=(
                        "Legacy target cluster count kept for compatibility. Final "
                        "HDBSCAN now runs once with the configured parameters."
                    ),
                    category="Clustering",
                    advanced=True,
                ),
                HyperparameterConfig(
                    name="hdbscan_scan_enabled",
                    display_name="HDBSCAN Final Scan",
                    param_type=ParamType.BOOLEAN,
                    default=False,
                    description=(
                        "Deprecated/ignored for final clustering. The final HDBSCAN "
                        "run uses the configured min_cluster_size/min_samples once."
                    ),
                    category="Clustering",
                ),
                HyperparameterConfig(
                    name="hdbscan_scan_eval_sample_size",
                    display_name="HDBSCAN Scan Eval Size",
                    param_type=ParamType.INTEGER,
                    default=3000,
                    min_value=200,
                    max_value=100000,
                    description=(
                        "Legacy parameter from the removed final HDBSCAN scan; kept "
                ...
                HyperparameterConfig(
                    name="hdbscan_scan_include_alternative_method",
                    display_name="HDBSCAN Scan Alt Method",
                    param_type=ParamType.BOOLEAN,
                    default=True,
                    description=(
                        "Legacy parameter from the removed final HDBSCAN scan; kept "
                ...
                HyperparameterConfig(
                    name="pseudo_leiden_resolution_strategy",
                    display_name="Pseudo Leiden Scan",
                    param_type=ParamType.CHOICE,
                    default="scraw_default",
                    choices=["scraw_default", "target_labels_silhouette"],
                    description=(
                        "Resolution search strategy used when pseudo_label_method=leiden. "
                        "`scraw_default` matches the original scRAW behavior."
                    ),
                    category="Pseudo Labels",
                    advanced=True,
                ),
                HyperparameterConfig(
                    name="pseudo_leiden_target_clusters",
                    display_name="Pseudo Leiden Target K",
                    param_type=ParamType.INTEGER,
                    default=0,
                    min_value=0,
                    max_value=300,
                    description="Optional explicit target cluster count for pseudo Leiden search (0 = auto).",
                    category="Pseudo Labels",
                    advanced=True,
                ),
                HyperparameterConfig(
                ...
                HyperparameterConfig(
                    ...
                    description=(
                        "Automatic K selection strategy when n_clusters=0. "
                        "The original scRAW default is `heuristic`."
                    ),
                    category="Pseudo Labels",
                    advanced=True,
                ),
    ...
    def __init__(self, params: Optional[Dict[str, Any]] = None) -> None:
        ...
        self._final_cell_weights: Optional[np.ndarray] = None
        self._final_cluster_component_weights: Optional[np.ndarray] = None
        self._final_density_component_weights: Optional[np.ndarray] = None
        self._final_clustering_info: Dict[str, Any] = {}
        self._dataset_n_classes: int = 0
        self._pseudo_clustering_info: Dict[str, Any] = {}
    ...
    def _path_param(self, key: str) -> Optional[Path]:
        ...
        if not text:
            return None
        # We normalize to an absolute path to avoid CWD ambiguities.
    ...
    def _save_training_checkpoint(
        ...
        *,
        ...
        # Full snapshot: enough information to resume training
        # without losing the dynamic state of weights/pseudo-labels.
        payload = {
            "version": 1,
    ...
    def _infer_batch_ids(self, data: Any, n_cells: int) -> Tuple[Optional[np.ndarray], int, Optional[str]]:
        ...
        # `want_batch` activates the search for batch column even if `use_batch=False`
        # as soon as a batch penalty (adv/mmd) is non-zero.
        ...
        if requested.lower() != "auto":
            ...
            # Automatic search on a list of frequent names.
            auto_keys = (
                "batch",
                "tech",
                "study",
        ...
        # Encoding string -> int for training the adversarial head.
    ...
    def _snapshot(
        ...
        """
        # Each snapshot contains embeddings + cell weights at the same epoch.
    ...
    def fit(self, data: Any, labels: Optional[Any] = None) -> "ScRAWAlgorithm":
        ...
        """
        # --- Step 1: deterministic initialization (reproducibility) ---
        seed = int(self._param("seed", self._param("random_state", 42)))
        self._set_seed(seed)
        self._embedding_snapshots = []
        self._loss_history = []
        self._final_clustering_info = {}
        self._pseudo_clustering_info = {}
        self._dataset_n_classes = 0

        # --- Step 2: input / target matrix preparation ---
        ...
        if labels is not None:
            try:
                ...
                if labels_arr.shape[0] == n_cells:
                    self._dataset_n_classes = int(np.unique(labels_arr).size)
            except Exception:
                self._dataset_n_classes = 0

        configured_n_clusters = int(self._param("n_clusters", 0) or 0)
        # Align the default behavior with the original scRAW implementation:
        # labels are not used to infer pseudo-K unless the user explicitly sets it.
        self.params["_pseudo_n_clusters"] = int(max(0, configured_n_clusters))
        ...
        if recon_mode == "nb":
            ...
            if nb_result is not None:
                ...
                # This trio explicitly aligns the NB training:
                # transformed input + raw counts target + size factors.
                X = X_model_nb
        ...
        # The batch adversarial head is only instantiated if:
        # - batch mode is requested,
        # - its weight is strictly positive,
        # - at least 2 batches exist (otherwise classification is impossible).
        if use_batch and adv_weight > 0.0 and n_batches >= 2:
            # Batch classification head used with gradient reversal.
        ...
        # --- Step 5: optimization configuration / planning ---
        ...
        raw_snapshot_epochs = self._param("snapshot_epochs", "")
        explicit_snapshot_epochs: set[int] = set()
        if raw_snapshot_epochs:
            if isinstance(raw_snapshot_epochs, (list, tuple, set)):
                raw_tokens = list(raw_snapshot_epochs)
            else:
                raw_tokens = str(raw_snapshot_epochs).replace(";", ",").split(",")
            for token in raw_tokens:
                try:
                    epoch_token = int(str(token).strip())
                except Exception:
                    continue
                if 0 <= epoch_token < epochs:
                    explicit_snapshot_epochs.add(epoch_token)
        ...
        def _should_capture_epoch(epoch_idx: int) -> bool:
            ...
            if epoch_idx in explicit_snapshot_epochs:
                return True
            # We always capture:
            # - the last epoch,
            # - the "anchor" epoch (end of warm-up),
            # - and then periodically after this anchor.
        ...
        # Reconstruction weights per cell (updated in the weighted phase).
        ...
        # Standard PyTorch DataLoader with shuffle for mini-batch training.
        ...
        # The dataset structure depends on the presence of NB size factors:
        ...
        if resume_checkpoint_path is not None:
            ...
            if load_optimizer_state and checkpoint.get("optimizer_state") is not None:
                try:
                    ...
                    # Non-blocking: we can restart with a fresh optimizer.
            ...
            if load_optimizer_state and checkpoint.get("scheduler_state") is not None:
                try:
                    ...
                    # Non-blocking: we align the scheduler phase below.
        ...
        if capture and epochs > 0 and start_epoch == 0 and not self._embedding_snapshots:
            # This snapshot serves as the "initial photo" for evolution plots.
        ...
        for epoch in range(start_epoch, epochs):
            ...
            for batch in loader:
                ...
                xb = xb.to(device)
                tb = tb.to(device)
                # `idx` allows retrieving global weights aligned with these cells.
        ...
        # --- Step 8: final embedding + final clustering ---
        self._embeddings = self._encode_full(X)
        self._labels = self._hdbscan_clustering(self._embeddings)
        self._final_cell_weights = np.asarray(current_weights, dtype=np.float32).copy()
        self._final_cluster_component_weights = np.asarray(
            current_cluster_component,
            dtype=np.float32,
        ).copy()
        self._final_density_component_weights = np.asarray(
            current_density_component,
            dtype=np.float32,
        ).copy()
        self._fitted = True

        # --- Step 9: losses recording ---
        history = []
        if warm_hist["epochs"]:
        ...
        # --- Step 10: traceability of effective parameters ---
        n_clusters_effective = int(
            self.params.get("_pseudo_n_clusters", 0)
            or self.params.get("unsupervised_k_selected", 0)
            or 0
        )
        final_clustering_info = dict(self._final_clustering_info or {})
        pseudo_clustering_info = dict(self._pseudo_clustering_info or {})
        pseudo_last = dict(pseudo_clustering_info.get("last") or {})
        resolved = {
            "seed": seed,
            "random_state": seed,
            ...
            "final_clustering_method_effective": final_clustering_info.get("method"),
            "final_clustering_selection_metric": final_clustering_info.get("selection_metric"),
            "final_clustering_selection_score": final_clustering_info.get("selection_score"),
            "final_clustering_target_clusters": final_clustering_info.get("target_clusters"),
            "final_clustering_target_source": final_clustering_info.get("target_source"),
            "rare_weighted_silhouette_effective": final_clustering_info.get("rare_weighted_silhouette"),
            "hdbscan_min_cluster_size_effective": final_clustering_info.get("min_cluster_size"),
            "hdbscan_min_samples_effective": final_clustering_info.get("min_samples"),
            "hdbscan_cluster_selection_method_effective": final_clustering_info.get("cluster_selection_method"),
            "hdbscan_noise_fraction_effective": final_clustering_info.get("noise_fraction"),
            "pseudo_leiden_resolution_strategy_effective": pseudo_clustering_info.get("strategy"),
            "pseudo_leiden_target_clusters_effective": pseudo_clustering_info.get("target_clusters"),
            "pseudo_leiden_resolution_last_effective": pseudo_last.get("resolution"),
        }
    ...
    def get_final_cell_weights(self) -> Optional[np.ndarray]:
        """Return the last training-time fused weight vector, even without snapshots."""
        if self._final_cell_weights is None:
            return None
        return np.asarray(self._final_cell_weights, dtype=np.float32).copy()

    def get_final_weight_components(self) -> Dict[str, Optional[np.ndarray]]:
        """Return the last training-time cluster/density weight components."""
        return {
            "cluster_component_weights": None
            if self._final_cluster_component_weights is None
            else np.asarray(self._final_cluster_component_weights, dtype=np.float32).copy(),
            "density_component_weights": None
            if self._final_density_component_weights is None
            else np.asarray(self._final_density_component_weights, dtype=np.float32).copy(),
        }

    def get_final_clustering_info(self) -> Dict[str, Any]:
        """Return metadata about the selected final clustering configuration."""
        return dict(self._final_clustering_info or {})

    def get_pseudo_clustering_info(self) -> Dict[str, Any]:
        """Return metadata about pseudo-clustering choices made during training."""
        return deepcopy(self._pseudo_clustering_info or {})
