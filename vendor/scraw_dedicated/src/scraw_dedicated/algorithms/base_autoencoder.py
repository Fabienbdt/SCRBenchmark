...
def gradient_reversal(x: torch.Tensor, lambda_: float = 1.0) -> torch.Tensor:
    ...
    # We explicitly move lambda to the correct device to avoid CPU/GPU mismatch.
...
class MLPAutoencoder(nn.Module):
    ...
    def __init__(self, shape: NetworkShape, dropout: float = 0.1) -> None:
        ...
        """
        super().__init__()
        # Encoder construction: progressive reduction towards the latent space.
        enc: List[nn.Module] = []
        ...
        # Decoder construction: symmetric to the encoder to reconstruct the input.
        dec: List[nn.Module] = []
        prev = shape.z_dim
...
def parse_hidden_layers(raw: Any) -> List[int]:
    ...
    """
    # Accepts either a Python list or a "512,256,128" string.
...
class BaseAutoencoderAlgorithm(BaseAlgorithm):
    ...
    def _as_numpy_matrix(self, data: Any) -> np.ndarray:
        ...
        # Supports AnnData (data.X) or direct numpy matrix.
    ...
    def _build_model(self, input_dim: int) -> MLPAutoencoder:
        ...
        # Reads network hyperparameters then instantiates the MLP autoencoder.
        shape = NetworkShape(
    ...
    def _encode_numpy(self, X: np.ndarray, batch_size: int = 2048) -> np.ndarray:
        ...
        # Mini-batch encoding to limit memory on large datasets.
