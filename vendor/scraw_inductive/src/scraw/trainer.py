...
def _combined_cell_weights(
    ...
    fusion_mode = str(config.weighting.weight_fusion_mode).strip().lower()
    if fusion_mode not in {"additive", "multiplicative"}:
        fusion_mode = "additive"

    cluster_power = max(0.0, float(config.weighting.cluster_weight_power))
    density_power = max(0.0, float(config.weighting.density_weight_power))
    cluster_scaled = np.power(np.maximum(cluster_weights, 1e-8), cluster_power).astype(
        np.float32,
        copy=False,
    )
    density_scaled = np.power(np.maximum(density_weights, 1e-8), density_power).astype(
        np.float32,
        copy=False,
    )

    if fusion_mode == "multiplicative":
        cluster_component_raw = cluster_scaled
        density_component_raw = density_scaled
        fused_raw = (cluster_scaled * density_scaled).astype(np.float32, copy=False)
    else:
        alpha = float(np.clip(config.weighting.cluster_density_alpha, 0.0, 1.0))
        cluster_component_raw = (alpha * cluster_scaled).astype(np.float32, copy=False)
        density_component_raw = ((1.0 - alpha) * density_scaled).astype(np.float32, copy=False)
