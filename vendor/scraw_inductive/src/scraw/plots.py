...
def plot_train_test_metrics(
    ...
    """Plot main train/test metrics side by side."""
    metric_names = [
        "NMI",
        "ARI",
        "ACC",
    ...
    for name in metric_names:
        if name not in train_metrics or name not in test_metrics:
            continue
        try:
            train_value = float(train_metrics[name])
            test_value = float(test_metrics[name])
        except (TypeError, ValueError):
            continue
        if not np.isfinite(train_value) or not np.isfinite(test_value):
            continue
        labels.append(name)
    ...
    ax.bar(x - width / 2, train_values, width, label="Train", color="#2563eb")
    ax.bar(x + width / 2, test_values, width, label="Test", color="#dc2626")
    ax.set_title("Train vs held-out test metrics")
