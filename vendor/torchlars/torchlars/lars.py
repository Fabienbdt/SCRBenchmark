...
Patched version: replaces the original C++/CUDA extension (torchlars._adaptive_lr)
with a pure-Python implementation to avoid CUDA toolkit version mismatches.

Original: https://github.com/kakaobrain/torchlars  (Apache-2.0)
"""
...
def compute_adaptive_lr(param_norm, grad_norm, weight_decay, eps, trust_coef, adaptive_lr):
    """Pure-Python replacement for the CUDA ``compute_adaptive_lr`` kernel.

    Formula (standard LARS):
        adaptive_lr = trust_coef * ||w|| / (||g|| + weight_decay * ||w|| + eps)

    Numerical safeguards:
        - Computation forced in float32 even when model runs in fp16/bf16.
        - Returns 1.0 when either norm is zero (no rescaling).
        - Clamps the result to avoid Inf / NaN propagation.
    """
    # Cast to float32 for numerical stability
    p_norm = float(param_norm)
    g_norm = float(grad_norm)

    if p_norm > 0.0 and g_norm > 0.0:
        denominator = g_norm + p_norm * weight_decay + eps
        lr = trust_coef * p_norm / denominator

        # Guard against Inf / NaN
        if not (lr == lr) or lr == float('inf'):  # NaN or Inf
            lr = 1.0

        adaptive_lr.fill_(lr)
    else:
        adaptive_lr.fill_(1.0)
